...
def seltb2():
    ...
    for x in cursor:
        L2.append(x)
    tuple=("covid_vaccine_database",)
    L2.remove(tuple)
    if len(L2)==0:
        return "You can't delete the default table."
    else:
        for i in range(1, len(L2) + 1):
            print(i, L2[i - 1][0])
...
while ans == "y":
    ...
    print("2.  Choose Another Database")
    print("3.  Create Table")
    print("4.  Add records in the Table")
    print("5.  Sort the Records and View")
    print("6.  View Records from the Table")
    print("7.  Edit records in the Table")
    print("8.  Delete Table/Database")
    print("9.  Exit")
    abc = "y"
    while abc == "y":
        while True:
    ...
    if y == 1:
        ...
        if len(datahoohoo)==0:
            ...
            print("You have the following Databases:")
            for x in datahoohoo:
                LDB2.append(x)
            for i in range(1, len(LDB2) + 1):
                print(i, LDB2[i - 1])
            print()
            ab = "y"
            while ab == "y":
                try:
                    tbnamex = int(
                        input(
                            "Which Database would you like to choose (Enter the number associated):"
                        )
                    )
                    tbnamex2 = tbnamex
                    if tbnamex > 0 and tbnamex <= len(LDB2):
                        ab = "n"
                        break
                    else:
                        print("There is some error. Please try Again.")
                    ...
                    ab = "y"
            print()
            database=LDB2[tbnamex - 1]
            database2=database
        myfile.close()
    else:
        if database2 == "PYTHON ROCKZ LOLZ":
            print("Please Create or Choose a Database First")
            print()
            ans = "y"
        else:
            ...
            if y == 8:
                ...
                if s == "database":
                    ...
                    myfile=open("Databases.txt","r")
                    myfile.seek(0)
                    ass=myfile.read()
                    LD=ass.split()
                    LD.remove(database)
                    myfile.close()
                    myfile=open("Databases.txt","w")
                    for j in LD:
                        myfile.write(j+"\n")
                    myfile.close()
                    LD=[]
                    ...
                    seldb()
                    tbname = seltb2()
                    if tbname=="You can't delete the default table.":
                        print(tbname)
                    else:
